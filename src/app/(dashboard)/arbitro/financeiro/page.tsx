...
export default async function ArbitroFinanceiroPage() {
  ...
  const { data: escs } = await supabase
    .from('escalacoes')
    .select('id, valor, pago, pago_em, jogo:jogos(data, mandante, visitante, competicao:competicoes(nome))')
    .eq('arbitro_id', profile?.id)
    .eq('status', 'confirmada')
    .order('pago', { ascending: true })

  type Row = { id: string; valor: number | null; pago: boolean; pago_em: string | null; jogo: { data: string; mandante: string; visitante: string; competicao: { nome: string } | null } | null }
                ...
                return (
                  <div key={l.id} className="flex items-center justify-between gap-3 rounded-xl border border-outline-variant/10 bg-surface px-4 py-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className={`text-sm font-bold ${l.pago ? 'text-green-700' : 'text-brand-orange-deep'}`}>{brl(Number(l.valor ?? 0))}</span>
                      {l.pago ? (
